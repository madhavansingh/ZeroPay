...
import { EvmBaseAdapter } from '../../src/adapters/chain/evmBaseAdapter';
...
describe('Chain Adapters Platform Infrastructure (Sprint 1)', () => {
  ...
  describe('ChainAdapterRegistry', () => {
    ...
    it('should register and resolve Base EVM adapter', () => {
      const adapter = chainAdapterRegistry.getAdapter('base');
      expect(adapter).toBeInstanceOf(EvmBaseAdapter);
      expect(adapter.chainName).toBe('Base');
      expect(adapter.nativeAssetSymbol).toBe('USDC');
    });
  ...
  describe('EvmBaseAdapter payload builders', () => {
    const base = new EvmBaseAdapter();

    it('builds EVM lock transaction JSON payload', async () => {
      const result = await base.buildLockTx('INV-123', 5000000, '0xcustomer');
      const payload = JSON.parse(result.txCbor);
      expect(payload.from).toBe('0xcustomer');
      expect(payload.to).toBe(result.scriptAddress);
      expect(payload.data).toContain(Buffer.from('INV-123').toString('hex'));
    });

    it('builds EVM release transaction JSON payload', async () => {
      const result = await base.buildReleaseTx('INV-123', 2, '0xcustomer');
      const payload = JSON.parse(result.txCbor);
      expect(payload.from).toBe('0xcustomer');
      expect(payload.data).toContain(Buffer.from('INV-123').toString('hex'));
    });

    it('builds EVM refund transaction JSON payload', async () => {
      const result = await base.buildRefundTx('INV-123', '0xcustomer');
      const payload = JSON.parse(result.txCbor);
      expect(payload.from).toBe('0xcustomer');
      expect(payload.data).toContain(Buffer.from('INV-123').toString('hex'));
    });

    it('builds EVM admin resolution transaction JSON payload', async () => {
      const result = await base.buildResolveTx('INV-123', 300, 200);
      const payload = JSON.parse(result.txCbor);
      expect(payload.data).toContain(Buffer.from('INV-123').toString('hex'));
    });

    it('verifies EVM payment on-chain successfully with 0x hash', async () => {
      const result = await base.verifyOnChainPayment('0xhash123', '0xmerchant', 500);
      expect(result.status).toBe('amount-matched');
      expect(result.totalPaid).toBe(500);
    });

    it('rejects EVM payment verification if transaction hash does not start with 0x', async () => {
      const result = await base.verifyOnChainPayment('non-evm-hash', '0xmerchant', 500);
